...
const solution = (input) => {
  const regex = /[0-9]/g;
  const justDigits = input.map((item) => {
    const match = item.match(regex);
    return match ? match.join('') : '';
  });

  const firstAndLast = justDigits.map((item) => {
    const first = item[0];
    const last = item[item.length - 1];
    return Number(first + last);
  });

  const result = firstAndLast.reduce((acc, item) => acc + item, 0);
  console.log(result);
};
