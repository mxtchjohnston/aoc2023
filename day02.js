...
const data = `Game 1: 1 blue, 2 green, 3 red; 7 red, 8 green; 1 green, 2 red, 1 blue; 2 green, 3 red, 1 blue; 8 green, 1 blue
...
const parseGames = data => {
  ...
  const gameData = games.map(game => { // [{red: 1, blue: 3, green: 3}, ]
    const [gameName, gameData] = game.split(':');
    const gameId = parseInt(gameName.split(' ')[1]);
    const gameRounds = gameData.split(';');
    const maxAmounts = {red: 0, blue: 0, green: 0};
    gameRounds.forEach(round => {
      const roundData = round.split(',').map(x => x.trim());
      roundData.forEach(round => {
        const [amount, color] = round.split(' ');
        maxAmounts[color] = Math.max(maxAmounts[color], parseInt(amount));
      });
    });
    return {gameId, maxAmounts};
  });
  return gameData;
};

const repeatedAdditon = arr => arr.reduce((acc, x) => acc + x, 0);

const part1 = data => {
  const gameData = parseGames(data);
  
  const standard = {red: 12, blue: 14, green: 13};
  const sum = gameData.map(({gameId, maxAmounts}) => {
    if (maxAmounts.red <= standard.red && maxAmounts.blue <= standard.blue && maxAmounts.green <= standard.green) {
      return gameId;
    }
    return 0;
...
const part2 = data => {
  const gameData = parseGames(data);
  const power = repeatedAdditon(gameData.map(({gameId, maxAmounts}) => maxAmounts.red * maxAmounts.blue * maxAmounts.green));
  return power;
};
