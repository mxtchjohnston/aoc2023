...
const matchAgainst = (arr) => new Set(arr.filter(c => c !== '.' && !isDigit(c)));
  ...
  gridOfChars.forEach((row, y) => {
    ...
    row.forEach((char, x) => {
      ...
      if (char === '.' && tmp.length > 0) {
        numbers.push({xl: x - tmp.length, xr: x - 1, y, value: parseInt(tmp), used: false});
        tmp = '';
        return;
      }
      ...
      if (isSymbol(char)) {
        ...
        numbers.push({xl: x, xr: x, y, value: char, used: false});
        tmp = '';
        return;
      }
    });
